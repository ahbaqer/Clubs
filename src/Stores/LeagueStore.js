...
createLeague = async (newLeague)=> {
    try {
        const response = await axios.post("ttp://localhost:9000/leagues" , newLeague);
...
updateLeague= async (updatedLeague) => {
    try{
       await axios.put(`http://localhost:9000/leagues/:${updatedLeague.id}`,updatedLeague) ;
       const league = this.leagues.find((league)=>league.id=== updatedLeague.id);
       for (const key in league ) {
           league[key]=upadtedLeague[key]
       }}
       catch(error){
           console.log(error)
       }
